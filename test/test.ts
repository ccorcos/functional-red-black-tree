...
function test(message: string, fn: (t: tape.Test) => Promise<void>) {
...
test("insert()", async function(t) {
	...
	for (var i = -20; i < 0; ++i) {
		...
		var ptr = 0
		await u.forEach(async function(k, v) {
			t.equals(k, arr[ptr++])
		})
...
test("iterators", async function(t) {
	...
	compareIterators(c, await u.begin(), t)
...
test("searching", async function(t) {
	...
	t.equals((await u.ge(3)).index, 6, "ge simple")
	t.equals((await u.ge(0.9)).index, 1, "ge run start")
	t.equals((await u.ge(1)).index, 1, "ge run mid")
	t.equals((await u.ge(1.1)).index, 5, "ge run end")
	t.equals((await u.ge(0)).index, 0, "ge first")
	t.equals((await u.ge(6)).index, 9, "ge last")
	t.equals((await u.ge(100)).valid, false, "ge big")
	t.equals((await u.ge(-1)).index, 0, "ge small")

	t.equals((await u.gt(3)).index, 7, "gt simple")
	t.equals((await u.gt(0.9)).index, 1, "gt run start")
	t.equals((await u.gt(1)).index, 5, "gt run mid")
	t.equals((await u.gt(1.1)).index, 5, "gt run end")
	t.equals((await u.gt(0)).index, 1, "gt first")
	t.equals((await u.gt(6)).valid, false, "gt last")
	t.equals((await u.gt(100)).valid, false, "gt big")
	t.equals((await u.gt(-1)).index, 0, "ge small")

	t.equals((await u.le(3)).index, 6, "le simple")
	t.equals((await u.le(0.9)).index, 0, "le run start")
	t.equals((await u.le(1)).index, 4, "le run mid")
	t.equals((await u.le(1.1)).index, 4, "le run end")
	t.equals((await u.le(0)).index, 0, "le first")
	t.equals((await u.le(6)).index, 10, "le last")
	t.equals((await u.le(100)).index, 10, "le big")
	t.equals((await u.le(-1)).valid, false, "le small")

	t.equals((await u.lt(3)).index, 5, "lt simple")
	t.equals((await u.lt(0.9)).index, 0, "lt run start")
	t.equals((await u.lt(1)).index, 0, "lt run mid")
	t.equals((await u.lt(1.1)).index, 4, "lt run end")
	t.equals((await u.lt(0)).valid, false, "lt first")
	t.equals((await u.lt(6)).index, 8, "lt last")
	t.equals((await u.lt(100)).index, 10, "lt big")
